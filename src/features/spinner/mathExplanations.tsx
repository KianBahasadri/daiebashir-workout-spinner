...
import {
    ...
    Sup,
    Var,
} from "../../shared/components/mathComponents";

export const MATH_EXPLANATIONS: Record<ExplanationKey, Explanation> = {
    exitProbability: {
        ...
        formula: (math: MathStats): MathFormula => ({
            general: (
                <>
                    <Var>p</Var> <Op>=</Op> Σ <Var>P</Var>(<Var>e</Var>
                    <Sub>exit</Sub>)
                </>
            ),
            substituted: (
                <>
                    <Var>p</Var> <Op>=</Op>{" "}
                    <Num>{math.exitProbability.toFixed(3)}</Num>
                </>
            ),
            result: (
                <>
                    <Op>=</Op>{" "}
                    <Num>{(math.exitProbability * 100).toFixed(1)}%</Num>
                </>
            ),
        }),
    },
    avgExercises: {
        ...
        formula: (math: MathStats): MathFormula => ({
            general: (
                <>
                    <Var>E</Var>[exercises] <Op>=</Op>{" "}
                    <Frac
                        num={
                            <>
                                <Num>1</Num> <Op>−</Op> <Var>p</Var>
                            </>
                        }
                        den={<Var>p</Var>}
                    />
            ...
            substituted: (
                <>
                    ...
                    <Frac
                        ...
                        den={<Num>{math.exitProbability.toFixed(3)}</Num>}
                    />
                </>
            ),
            result: (
                        ...
                        num={
                            <Num>
                                {(
                                    ...
                                    (1 - math.exitProbability)
                                ).toFixed(3)}
                            </Num>
                        }
                        den={<Num>{(1 - math.exitProbability).toFixed(3)}</Num>}
                    />
                </>
            ),
            result: (
                <>
                    <Op>=</Op>{" "}
                    <Num>{math.expectedDurationPerSpin.toFixed(1)} min</Num>
                </>
            ),
        }),
    },
    totalDuration: {
        ...
        formula: (math: MathStats): MathFormula => ({
            general: (
                <>
                    <Var>T</Var> <Op>=</Op> <Var>E</Var>[exercises] <Op>×</Op>{" "}
                    <Var>D̄</Var>
                </>
            ),
            substituted: (
                <>
                    ...
                    <Op>×</Op>{" "}
                    <Num>{math.expectedDurationPerSpin.toFixed(1)}</Num>
                </>
            ),
            result: (
                <>
    ...
    shawarma: {
        ...
        formula: (math: MathStats): MathFormula => ({
            general: (
                <>
                    <Var>E</Var>[workouts] <Op>=</Op>{" "}
                    <Frac
                        num={
                            <>
                                <Var>P</Var>(<Var>exit</Var>)
                            </>
                        }
                        den={
                            <>
                                <Var>P</Var>(<Var>shawarma</Var>)
                            </>
                        }
                    />
                </>
            ...
            substituted: (
                <>
                    <Var>E</Var>[workouts] <Op>=</Op>{" "}
                    <Frac
                        num={<Num>{math.exitProbability.toFixed(3)}</Num>}
                        den={<Num>{math.shawarmaWeight.toFixed(3)}</Num>}
                    />
                </>
            ),
            result: (
    ...
    lengthCurve: {
        ...
        formula: (math: MathStats): MathFormula => ({
            general: (
                <>
                    <Var>P</Var>(L = k) <Op>=</Op> (1 − <Var>p</Var>)
                    <Sup>
                        <Var>k</Var>−1
                    </Sup>{" "}
                    <Op>×</Op> <Var>p</Var>
            ...
            substituted: (
                <>
                    <Var>P</Var>(L = 5) <Op>=</Op> (1 −{" "}
                    <Num>{math.exitProbability.toFixed(3)}</Num>)<Sup>4</Sup>{" "}
                    <Op>×</Op> <Num>{math.exitProbability.toFixed(3)}</Num>
                </>
            ),
            result: (
                <>
                    ...
                    <Num>
                        {(
                            Math.pow(1 - math.exitProbability, 4) *
                            math.exitProbability *
                            100
                        ).toFixed(1)}
                        %
                    </Num>
    ...
    cdf: {
        ...
        formula: (math: MathStats): MathFormula => ({
            general: (
                <>
                    <Var>P</Var>(L ≤ n) <Op>=</Op> 1 − (1 − <Var>p</Var>)
                    <Sup>
                        <Var>n</Var>
                    </Sup>
                </>
            ...
            substituted: (
                <>
                    <Var>P</Var>(L ≤ 10) <Op>=</Op> 1 − (1 −{" "}
                    <Num>{math.exitProbability.toFixed(3)}</Num>)<Sup>10</Sup>
                </>
            ),
            result: (
                <>
                    <Op>=</Op>{" "}
                    <Num>{(math.chanceEndWithin(10) * 100).toFixed(1)}%</Num>
                </>
            ),
        }),
    },
                    ...
                    <>
                        ...
                        <Frac
                            num={
                                <Num>
                                    {sampleGroup
                                        ? sampleGroup.groupProbability.toFixed(
                                              3,
                                          )
                                        : "N/A"}
                                </Num>
                ...
                result: (
                    <>
                        ...
                        <Num>
                            {sampleGroup
                                ? Number.isFinite(
                                      sampleGroup.expectedHitsPerWorkout,
                                  )
                                    ? sampleGroup.expectedHitsPerWorkout.toFixed(
                                          2,
                                      )
                                    : "∞"
                                : "N/A"}
                ...
                substituted: (
                    <>
                        ...
                        <Frac
                            ...
                            den={
                                <Num>
                                    {Number.isFinite(hitsPerWorkout)
                                        ? hitsPerWorkout.toFixed(2)
                                        : "∞"}
                                </Num>
                            }
                        />
