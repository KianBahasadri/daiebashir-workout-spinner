...
const buildWeightedWheelSlots = (exercises: Exercise[]) => {
    ...
    const plans = exercises.map((exercise) => {
        ...
        const perItemProbability =
            RARITY_CONFIG[exercise.rarity].probability / tierSize;
...
export function SpinnerApp() {
    const [shuffledExercises] = useState<Exercise[]>(() =>
        shuffleArray(EXERCISES),
    );
    const [activePopup, setActivePopup] = useState<ExplanationKey | null>(null);
    const [mathTab, setMathTab] = useState<
        "sessions" | "exercises" | "game" | "rules" | "simulation"
    >("game");
    ...
    const wheelSlots = useMemo(
        () => buildWeightedWheelSlots(shuffledExercises),
        [shuffledExercises],
    );
    const {
        unlockedTabsCount,
    ...
    useEffect(() => {
        ...
        const loop = () => {
            ...
            const currentTranslateX =
                getTranslateXFromComputedTransform(trackEl);
    ...
    return (
        <div className="app">
            <GameHeader
                title="Shawarma Spinner"
                switchTo="/snakes-ladders"
                switchLabel="Snakes & Ladders"
            />
                ...
                <div className="reel-window" ref={reelRef}>
                    <div
                        ...
                        style={{
                            ...
                            opacity: reelWidth > 0 ? 1 : 0,
                            transition: isSpinning
                                ? `transform ${SPIN_DURATION_MS}ms cubic-bezier(0.2, 0.9, 0.2, 1), opacity 0.5s ease-in-out`
                    ...
                    >
                        {trackItems.map((exercise, index) => (
                            <div
                                ...
                                style={{
                                    backgroundColor:
                                        exercise.rarity === "legendary" ||
                                        exercise.rarity === "godly"
                                            ? undefined
                                            : exercise.color,
                                }}
                            >
                                <div className="exercise-name-label">
                                    {exercise.name}
                                </div>
                                <div className="rarity-label">
                                    {
                                        RARITY_CONFIG[
                                            exercise.rarity as keyof typeof RARITY_CONFIG
                                        ].name
                                    }
                                </div>
                            </div>
