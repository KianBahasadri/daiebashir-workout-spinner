...
import { type MathStats, type ExplanationKey } from "../../../shared/types";
import { formatPercent } from "../../../shared/utils";
import { spinsForCdf } from "../../../shared/mathUtils";
...
export function SessionsTab({
    ...
    const medianSpinsLabel = Number.isFinite(medianSpins)
        ? `${medianSpins}`
        : "∞";
    const medianChance = Number.isFinite(medianSpins)
        ? math.chanceEndWithin(medianSpins)
        : 0;
    ...
    const expectedSpinsLabel = Number.isFinite(expectedSpins)
        ? `${expectedSpins}`
        : "∞";
    const expectedChance = Number.isFinite(expectedSpins)
        ? math.chanceEndWithin(expectedSpins)
        : 0;
        ...
        <div className="math-grid">
            <div className="math-card math-card--full">
                ...
                <dl className="math-kpis">
                    <div className="math-kpi">
                        <dt>
                            Average exercises before{" "}
                            <span className="shawarma-text">shawarma</span>
                            <InfoPopup
                                explanationKey="avgExercises"
                                activePopup={activePopup}
                                setActivePopup={setActivePopup}
                                math={math}
                                explanations={MATH_EXPLANATIONS}
                            />
                        </dt>
                        <dd>
                    ...
                    <div className="math-kpi">
                        <dt>
                            Average workout duration before{" "}
                            <span className="shawarma-text">shawarma</span>
                            <InfoPopup
                                explanationKey="totalDuration"
                                activePopup={activePopup}
                                setActivePopup={setActivePopup}
                                math={math}
                                explanations={MATH_EXPLANATIONS}
            ...
            <div className="math-card math-card--full">
                ...
                <p className="math-subtitle">
                    Likelihood the workout ends on spin{" "}
                    <span className="math-mono">k</span> (geometric
                    distribution). Mean is{" "}
                    <span className="math-mono">E[L] = 1 / p</span>. Expected
                    duration:{" "}
                ...
                {(() => {
                    const lengthCurvePoints = Math.max(
                        1,
                        math.lengthCurve.length,
                    );
                    const chartW = 800;
                    const chartH = 200;
                    const pad = 20;
                    ...
                    const maxProb = Math.max(
                        ...math.lengthCurve.map(
                            (point) => point.probabilityEndOnSpin,
                        ),
                        0,
                    );
                    ...
                    const points = math.lengthCurve
                        .map((point, idx) => {
                            const t =
                                math.lengthCurve.length > 1
                                    ? idx / (math.lengthCurve.length - 1)
                                    : 0;
                            const x = pad + t * innerW;
                            const y =
                                pad +
                                innerH -
                                (point.probabilityEndOnSpin / safeMax) * innerH;
                    ...
                    const expectedT = Number.isFinite(expected)
                        ? Math.max(
                              ...
                              Math.min(
                                  1,
                                  (expected - 1) / (lengthCurvePoints - 1),
                              ),
                          )
                        : 1;
                    ...
                    const xStep = Math.max(
                        2,
                        Math.floor(lengthCurvePoints / 10),
                    );
                    ...
                    if (
                        xAxisLabels[xAxisLabels.length - 1]?.value !==
                        lengthCurvePoints
                    ) {
                        const t =
                            (lengthCurvePoints - 1) / (lengthCurvePoints - 1);
                        xAxisLabels.push({
                            value: lengthCurvePoints,
                            x: pad + t * innerW,
                        });
                    }
                ...
                <p className="math-subtitle">
                    Total probability the workout has ended by spin{" "}
                    <span className="math-mono">n</span>. Targeting{" "}
                    <span className="math-mono">95%</span> confidence at spin{" "}
                    <span className="math-mono">{math.lengthCurve.length}</span>
                    .
                </p>
                ...
                {(() => {
                    ...
                    const points = math.lengthCurve
                        .map((point, idx) => {
                            ...
                            const y =
                                pad +
                                innerH -
                                (point.cumulativeProbability / safeMax) *
                                    innerH;
                            return `${x.toFixed(2)},${y.toFixed(2)}`;
                        })
                        .join(" ");
                    ...
                    const yAxisLabels = [0, 0.25, 0.5, 0.75, 1.0].map(
                        (prob) => ({
                            value: prob,
                ...
                <div
                    ...
                    aria-label="Chance the workout has ended within N spins"
                >
                    <div>
                        <span className="math-mono">
                            P(L ≤ {medianSpinsLabel})
                        </span>
                        : {formatPercent(medianChance)}
                        <span className="math-duration">
                            ≤ {medianDurationLabel} min
                        </span>
                    </div>
                    <div>
                        <span className="math-mono">
                            P(L ≤ {expectedSpinsLabel})
                        </span>
                        : {formatPercent(expectedChance)}
                        <span className="math-duration">
                            ≤ {expectedDurationLabel} min
                        </span>
                    </div>
                    <div>
                        <span className="math-mono">
                            P(L ≤ {math.lengthCurve.length})
                        </span>
                        :{" "}
                        {formatPercent(
                            math.chanceEndWithin(math.lengthCurve.length),
                        )}
                        <span className="math-duration">
                            ≤{" "}
                            {(
                                Math.max(0, math.lengthCurve.length - 1) *
                                math.expectedDurationPerSpin
                            ).toFixed(0)}{" "}
                            min
                        </span>
                    </div>
