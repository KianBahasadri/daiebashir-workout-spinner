...
const buildProbabilityGrid = (math: MathStats, totalBoxes: number) => {
    ...
    if (
        ...
        return {
            cells: [],
            countsByRarity: {} as Record<string, number>,
            countsByExercise: {} as Record<string, number>,
        };
    }
    ...
    for (const plan of plans) {
        countsByRarity[plan.exercise.rarity] =
            (countsByRarity[plan.exercise.rarity] ?? 0) + plan.copies;
    ...
    const sortedPlans = [...plans].sort(
        (a, b) => rarityRank[a.exercise.rarity] - rarityRank[b.exercise.rarity],
    );
                ...
                {probabilityGrid.cells.length > 0 && (
                    <div className="probability-grid-section">
                        <div className="probability-grid-header">
                            <h4 className="probability-grid-title">
                                Probability grid ({PROBABILITY_GRID_BOXES}{" "}
                                boxes)
                            </h4>
                            <p className="math-subtitle">
                                Each box represents ~
                                {formatPercent(1 / PROBABILITY_GRID_BOXES)}{" "}
                                chance per spin.
                                <br />
                                <span
                                    style={{
                        ...
                        <div
                            className="probability-grid-legend"
                            aria-label="Probability grid legend"
                        >
                            {math.groupedByRarity
                                ...
                                .map((group) => {
                                    const rarityConfig =
                                        RARITY_CONFIG[group.rarity];
                                    const boxCount =
                                        probabilityGrid.countsByRarity[
                                            group.rarity
                                        ] ?? 0;
                                    return (
                                        <div
                                            key={group.rarity}
                                            className="probability-grid-legend-item"
                                        >
                                            <span
                        ...
                        <div
                            ...
                            aria-label={`Probability grid with ${PROBABILITY_GRID_BOXES} boxes`}
                        >
                            {probabilityGrid.cells.map((cell, index) => {
                                const rarityConfig =
                                    RARITY_CONFIG[
                                        cell.rarity as keyof typeof RARITY_CONFIG
                                    ];
                                return (
                                    <span
