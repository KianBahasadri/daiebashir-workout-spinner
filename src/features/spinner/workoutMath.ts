import { geometricCdf, spinsForCdf } from "../../shared/mathUtils";
import {
    type Exercise,
    type MathStats,
    RARITY_CONFIG,
...
export function calculateMathStats(exercises: Exercise[]): MathStats {
    ...
    const exitExercises = exercises.filter(
        (exercise) => exercise.isExitCondition,
    );
    ...
    const shawarmaExercise = exercises.find(
        (exercise) => exercise.rarity === "legendary",
    );
    const shawarmaWeight = shawarmaExercise
        ? RARITY_CONFIG.legendary.probability
        : 0;
    const shawarmaGivenExit =
        exitProbability > 0 ? shawarmaWeight / exitProbability : 0; // Probability of shawarma given exit
    const expectedWorkoutsUntilShawarma =
        shawarmaGivenExit > 0
            ? 1 / shawarmaGivenExit
            : Number.POSITIVE_INFINITY; // Workouts until specific legendary shawarma hit

    const usesUniformFallback = false; // We now have defined probabilities

    const expectedSpinsUntilEnd =
        exitProbability > 0 ? 1 / exitProbability : Number.POSITIVE_INFINITY;
    const expectedExercisesBeforeEnd =
        exitProbability > 0
            ? (1 - exitProbability) / exitProbability
            : Number.POSITIVE_INFINITY;

    // Calculate expected duration per non-exit spin
    const nonExitExercises = exercises.filter(
        (exercise) => !exercise.isExitCondition,
    );
    ...
    const exercisesByRarity = nonExitExercises.reduce(
        (acc, exercise) => {
    ...
    for (const rarity of Object.keys(
        RARITY_CONFIG,
    ) as (keyof typeof RARITY_CONFIG)[]) {
        ...
        if (exercisesInTier.length > 0) {
            ...
            const avgDurationInTier =
                exercisesInTier.reduce((sum, ex) => sum + ex.duration, 0) /
                exercisesInTier.length;
            weightedDurationSum += tierProbability * avgDurationInTier;
            totalDurationWeight += tierProbability;
        }
    }

    const expectedDurationPerSpin =
        totalDurationWeight > 0 ? weightedDurationSum / totalDurationWeight : 0;
    ...
    const groupedByRarity = Array.from(rarityGroupsMap.entries())
        .map(([rarityKey, exercisesInGroup]) => {
            ...
            const perItemProbability =
                exercisesInGroup.length > 0
                    ? rarityProbability / exercisesInGroup.length
                    : 0;
            const groupProbability = rarityProbability;

            // Check if this group contains exit conditions
            const hasExitConditions = exercisesInGroup.some(
                (e) => e.isExitCondition,
            );

            const expectedHitsPerWorkout =
                exitProbability > 0 ? rarityProbability / exitProbability : 0;
    ...
    const lengthCurve = Array.from({ length: lengthCurvePoints }, (_, i) => {
        ...
        const probabilityEndOnSpin =
            exitProbability > 0
                ? Math.pow(1 - exitProbability, spins - 1) * exitProbability
                : 0;
...
export function pickExerciseIndex(exercises: Exercise[]): number {
    ...
    const rarityOrder: (keyof typeof RARITY_CONFIG)[] = [
        "common",
        "rare",
        "epic",
        "legendary",
    ...
    for (const rarity of rarityOrder) {
        if (exercisesByRarity[rarity]) {
            ...
            if (roll < cumulativeProbability) {
                ...
                const exercisesInTier = exercisesByRarity[rarity];
                const randomIndexInTier = Math.floor(
                    Math.random() * exercisesInTier.length,
                );
