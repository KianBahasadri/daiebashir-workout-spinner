...
export function useGameState({ exercises, wheelSlots, initAudioContext }: UseGameStateArgs) {
  const [unlockedTabsCount, setUnlockedTabsCount] = usePersistentState<number>('unlockedTabsCount', 0)
  ...
  const spin = () => {
    if (isSpinning || !reelWidth || exercises.length === 0 || wheelSlots.length === 0) return
    ...
    timeoutRef.current = window.setTimeout(() => {
      ...
      setUnlockedTabsCount((prev) => Math.min(prev + 1, MAX_UNLOCKED_TABS))
      setIsSpinning(false)
    }, SPIN_DURATION_MS)
  }
